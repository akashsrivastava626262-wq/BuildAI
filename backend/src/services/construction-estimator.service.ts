...
export function calculateConstructionEstimate(
  ...
  const plotAreaSqFt = input.plotLengthFt * input.plotBreadthFt
  ...
  if (input.foundationType === 'isolated_footing') {
    ...
    excavationVolumeCum = input.columnCount! * footingL * footingW * depth * 1.1
    methodology.push(
    ...
    const widthM = input.stripFootingWidthFt! * FT_TO_M
    const depthM = input.foundationDepthFt * FT_TO_M
    excavationVolumeCum = perimeterFt * FT_TO_M * widthM * depthM * 1.1
    methodology.push(`Strip excavation = perimeter × footing_width × depth × 1.10`)
  } else {
    const raftThicknessM = input.raftThicknessMm! * MM_TO_M
    excavationVolumeCum = builtUpSqM * (input.foundationDepthFt * FT_TO_M) * 1.1
